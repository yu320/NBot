...
# ✅ 1. 新增關鍵字檔案的路徑
#
MEETUP_KEYWORDS_FILE = './data/meetup_keywords.json' 
...
class Meetup(Cog_Extension):
    ...
    def __init__(self, bot):
        ...
        # ✅ 2. 在啟動時載入關鍵字
        #
        self.meetup_keywords: List[str] = [] # 儲存關鍵字的清單
        self._load_keywords() # 呼叫載入函式


    # --- JSON 輔助函式 (Meetup List) ---
    ...
    def _load_keywords(self):
        """載入關鍵字清單"""
        try:
            if not os.path.exists(MEETUP_KEYWORDS_FILE):
                # 如果檔案不存在，建立一個預設的
                default_keywords = ["想要吃", "想去吃", "想吃"]
                self._save_keywords(default_keywords)
                self.meetup_keywords = default_keywords
                logging.warning(f"{MEETUP_KEYWORDS_FILE} 不存在，已建立預設檔案。")
            else:
                with open(MEETUP_KEYWORDS_FILE, 'r', encoding='utf8') as f:
                    self.meetup_keywords = json.load(f)
                    logging.info(f"已成功從 {MEETUP_KEYWORDS_FILE} 載入 {len(self.meetup_keywords)} 個關鍵字。")
                    
        except Exception as e:
            logging.error(f"載入 {MEETUP_KEYWORDS_FILE} 失敗: {e}")
            self.meetup_keywords = ["想要吃"] # 載入失敗時的備用方案

    def _save_keywords(self, keywords: List[str]):
        """儲存關鍵字清單"""
        try:
            with open(MEETUP_KEYWORDS_FILE, 'w', encoding='utf8') as f:
                json.dump(keywords, f, indent=4, ensure_ascii=False)
        except Exception as e:
            logging.error(f"儲存 {MEETUP_KEYWORDS_FILE} 失敗: {e}")
    # ---
    ...
    async def on_command_error(self, ctx, error):
        ...
        if ctx.command and (ctx.command.name == 'eat' or (ctx.command.root_parent and ctx.command.root_parent.name == 'eat')):
            ...
            if isinstance(error, commands.MissingRequiredArgument):
                param_name_tw = "參數"
                if error.param.name == 'title': param_name_tw = "戰鬥邀請標題"
                elif error.param.name == 'location': param_name_tw = "地點"
                elif error.param.name == 'message_id': param_name_tw = "戰鬥邀請訊息ID"
                elif error.param.name == 'new_location': param_name_tw = "新地點"
    ...
    async def on_message(self, msg: discord.Message):
        ...
        # ✅ 4. 修改檢查邏輯
        #
        # 將訊息轉為小寫，並檢查 "任何" 在 self.meetup_keywords (從 JSON 載入) 中的字
        # 是否 "存在於" 訊息內容中
        #
    ...
    async def eat(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            ...
            embed.add_field(name=f"4. 重新載入關鍵字 (僅限管理員)", value=f"`{prefix}eat reload_keywords`", inline=False) # ✅ 5. 新增說明
                ...
                if meetup_message.embeds:
                    ...
                    class FakeAuthor:
                        def __init__(self, creator_id, guild):
                            self.id = creator_id; self.guild = guild; self.display_name = f"User (ID: {creator_id})"; self.avatar = None
                            try:
                                member = guild.get_member(creator_id)
                                if member: self.display_name = member.display_name; self.avatar = member.avatar
                            except: pass
                    class FakeContext:
    ...
    async def reload_keywords(self, ctx: commands.Context):
        ...
        try:
            self._load_keywords()
            await ctx.send(f"✅ 成功重新載入 {len(self.meetup_keywords)} 個關鍵字。", ephemeral=is_private)
        except Exception as e:
            await ctx.send(f"❌ 重新載入關鍵字失敗: {e}", ephemeral=is_private)
